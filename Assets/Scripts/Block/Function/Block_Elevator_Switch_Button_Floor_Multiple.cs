...
public class Block_Elevator_Switch_Button_Floor_Multiple : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            if (linkedSwitches[i].GetComponent<Block_Elevator_Switch_Button_Floor_Multiple>())
            {
                linkedSwitches[i].GetComponent<Block_Elevator_Switch_Button_Floor_Multiple>().ButtonAnimation_On();
            }
        }
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        for (int i = 0; i < linkedSwitches.Count; i++)
        {
            if (linkedSwitches[i].GetComponent<Block_Elevator_Switch_Button_Floor_Multiple>())
            {
                linkedSwitches[i].GetComponent<Block_Elevator_Switch_Button_Floor_Multiple>().ButtonAnimation_Off();
            }
        }
    ...
    IEnumerator DelayActivation(float waitTime, bool activationState)
    {
        for (int i = 0; i < linkedSwitches.Count; i++)
        {
            if (linkedSwitches[i].GetComponent<Block_Elevator_Switch_Button_Floor_Multiple>())
            {
                linkedSwitches[i].GetComponent<Block_Elevator_Switch_Button_Floor_Multiple>().elevatorIsActivated = activationState;
            }
        }
